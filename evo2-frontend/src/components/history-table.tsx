import { historicalRankingPolicy } from "~/lib/disease-ranking-policy";
...
function toPredictionHistoryRecord(
  ...
  return {
    id:
      readString(row, ["id"]) ?? `${createdAt}-${variantPosition ?? "unknown"}`,
...
export async function HistoryTable({ userId }: { userId: string }) {
  ...
  const { data: history, error } = await fetchPredictionHistory(
    supabase,
    userId,
  );
  ...
  if (error) {
    ...
    return (
      <div className="p-4 text-red-400">Error loading history records.</div>
    );
  }
  ...
  if (!history || history.length === 0) {
    return (
      <div className="text-muted-foreground py-12 text-center">
        ...
        <Link
          href="/"
          className="text-phosphor mt-2 inline-block hover:underline"
        >
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border/50 hover:bg-transparent">
            ...
            <TableHead>Evo2 computational prediction</TableHead>
        ...
        <TableBody>
          {records.map((record) => (
            <TableRow
              key={record.id}
              className="border-border/40 hover:bg-muted/50 border-b transition-colors"
            >
