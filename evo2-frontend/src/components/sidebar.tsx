...
export const Sidebar = memo(function Sidebar({ isCollapsed, setIsCollapsed }: SidebarProps) {
  ...
  if (loading && !user) {
    return (
        <aside 
            className={`fixed inset-y-0 left-0 z-50 flex flex-col bg-black/80 backdrop-blur-xl border-r border-white/10 ${isCollapsed ? 'w-[72px]' : 'w-64'} animate-pulse`}
            aria-hidden="true"
        >
             <div className="h-16 border-b border-white/10" />
             <div className="flex-1 p-4 space-y-4">
                 {[...Array(5)].map((_, i) => <div key={i} className="h-8 bg-white/5 rounded" />)}
             </div>
        </aside>
    )
  }
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 rounded-md bg-black/50 border border-white/10 p-2 text-foreground lg:hidden focus:outline-none focus:ring-2 focus:ring-phosphor backdrop-blur-md"
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col bg-black/80 backdrop-blur-xl border-r border-white/10 shadow-2xl transition-all duration-300 ease-in-out lg:translate-x-0 ${
      ...
      >
        ...
        {/* Footer Area */}
        <div className={`p-4 border-t border-white/10 flex flex-col gap-4 ${isCollapsed ? 'items-center' : ''}`}>
          ...
          <button
            ...
            className={`hidden lg:flex items-center justify-center p-2 rounded-md text-muted-foreground hover:bg-white/5 hover:text-foreground transition-colors ${
